          ...
          <div class="modal-body" v-if="currentStore">
            ...
            <table>
              ...
              <tbody>
                <tr>
                  <th>早上</th>
                  <td v-for="(sp, index) in servicePeriods[0]" :key="`m_${index}`">{{ sp }}</td>
                </tr>
                <tr>
                  <th>中午</th>
                  <td v-for="(sp, index) in servicePeriods[1]" :key="`a_${index}`">{{ sp }}</td>
                </tr>
                <tr>
                  <th>晚上</th>
                  <td v-for="(sp, index) in servicePeriods[2]" :key="`n_${index}`">{{ sp }}</td>
                </tr>
              </tbody>
            </table>

            <h2 class="title">地址: {{ currentStore.address }}</h2>
            <h2 class="title">電話: {{ currentStore.phone }}</h2>
            <h2 v-if="currentStore.custom_note" class="title">備註: {{ currentStore.custom_note }}</h2>
